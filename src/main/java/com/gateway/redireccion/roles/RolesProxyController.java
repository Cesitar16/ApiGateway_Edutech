...
@RestController
@RequestMapping("/api/proxy/roles")
...
public class RolesProxyController {
    ...
    public ResponseEntity<?> proxyRoles(HttpServletRequest request,
                                        @RequestBody(required = false) String body,
                                        @RequestHeader HttpHeaders headers) {

        String originalPath = request.getRequestURI().replace("/api/proxy/roles", "");
        String targetUrl = "http://localhost:8001/api/roles" + originalPath;
        ...
        if (method == HttpMethod.DELETE) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede eliminar roles\"}");
            }
        }
