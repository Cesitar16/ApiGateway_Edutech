package com.gateway.redireccion.pagos;
...
@RestController
@RequestMapping("/api/proxy/pagos")
...
public class PagosProxyController {
    ...
    public ResponseEntity<?> proxyPagos(HttpServletRequest request,
        ...
        String originalPath = request.getRequestURI().replace("/api/proxy/pagos", "");
        String targetUrl = "http://localhost:8005/api/pagos" + originalPath;
        ...
        if (method == HttpMethod.DELETE) {
            ...
            if (!"admin".equalsIgnoreCase(rol)) {
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body("{\"error\": \"Solo admin puede eliminar pagos\"}");
            }
        }
